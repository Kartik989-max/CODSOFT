...
$sql="SELECT * FROM blog LEFT JOIN user ON blog.authod_id=user.user_id ORDER BY blog.publish_date ASC";
$sql2="SELECT * FROM blog ORDER BY blog.publish_date DESC limit $offset,$limit";
...
<body>
    ...
    <div class="main">
        <a href="main.php">
        <div class="main_head">
        <h1>Captured Imagination</h1>
        <h3>Journey into poetic realms, where <a class="Soul"> souls</a> weave tales with words.</h3>
            </div>
            <div class="main_feature" >
            <div class="main_feature_bigpost" style="    padding: 1rem 0px;
    text-align: center;">
                <img src="./img/Screenshot 2023-08-05 005333.png" alt="">
                <h2>5 Facts About the Space that no one told you</h2>
                <p style="font-size: 0.8rem;
    margin: 0px 15%;
    /* margin-right: 10rem; */
    opacity: 65%;
    font-weight: 300;">Vastness of Space: Space is incomprehensibly vast. The observable universe is estimated to be around 93 billion light-years in diameter. <a href="" class="Soul" style="cursor: pointer; text-decoration: none;"> Read More</a></p>
    </a>
            </div>
    ...
    <?php 
        if($row){  
            $count=0;
            while($result=mysqli_fetch_assoc($run) and $count<4){
                ?>    
            <p> <a href="./blog.php?id=<?= $result['blog_id']?>" style="font-size:1rem;cursor: pointer; text-decoration: none;">
                <?= ucfirst($result['title'])?></p></a>
     <?php $date=$result['publish_date'];?>
     <a href="./blog.php?id=<?= $result['blog_id']?>" class="Soul" style="cursor: pointer; text-decoration: none;">Read More</a>
    ...
    <h1 style="text-align:center;font-size:2.4rem;">Recent Posts</h1>
    ...
    background: #FE4233;
    cursor: pointer;" href="index.php?page=<?=$i?>">
        <?=$i?>
    </a>
    </li>
        <?php } ?>
